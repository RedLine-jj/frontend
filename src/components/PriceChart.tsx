...
const SITE_COLORS: Record<string, string> = {
  modeman: 'hsl(38 80% 55%)',
  musinsa: 'hsl(220 70% 55%)',
  '29cm': 'hsl(152 60% 42%)',
};
...
export default function PriceChart({ data, listPrice }: PriceChartProps) {
  ...
  // Combined: pivot data so each date row has site prices as columns
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold font-display text-gradient">가격 추이</h3>
        </div>
        <div className="flex items-center gap-2 text-xs">
          <span className="text-muted-foreground">현재 최저가:</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        {stats.map(s => (
          <div key={s.site} className="rounded-lg bg-secondary/50 p-3 space-y-1">
            ...
            <p className="text-[10px] text-muted-foreground">
              최저 {formatPrice(s.min)} · 최고 {formatPrice(s.max)}
            </p>
          </div>
        ))}
      </div>
      ...
      <Tabs value={view} onValueChange={v => setView(v as any)}>
        ...
        <TabsContent value="combined" className="mt-3">
          <ChartContainer config={chartConfig} className="h-[240px] w-full">
            <LineChart data={combinedData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <XAxis dataKey="date" tickFormatter={formatDate} tick={{ fontSize: 10, fill: 'hsl(220 10% 52%)' }} />
              <YAxis tickFormatter={v => `${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 10, fill: 'hsl(220 10% 52%)' }} domain={['dataMin - 5000', 'dataMax + 5000']} />
        ...
        <TabsContent value="trend" className="mt-3">
          <ChartContainer config={{ avg: { label: '평균가', color: 'hsl(var(--primary))' } }} className="h-[240px] w-full">
            <AreaChart
            ...
            >
              ...
              <YAxis tickFormatter={v => `${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 10, fill: 'hsl(220 10% 52%)' }} domain={['dataMin - 5000', 'dataMax + 5000']} />
              <ChartTooltip content={<ChartTooltipContent formatter={(value) => formatPrice(value as number)} />} />
              <Area type="monotone" dataKey="avg" stroke="hsl(38 80% 55%)" fill="url(#avgGrad)" strokeWidth={2} />
            </AreaChart>
          </ChartContainer>
        </TabsContent>
              ...
              <defs>
                {sites.map(site => (
                  <linearGradient key={site} id={`grad-${site}`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={SITE_COLORS[site]} stopOpacity={0.2} />
